...
EF_DRIVER_STATUS EF_UART_enable(EF_UART_TYPE_PTR uart){
    ...
    if (uart == NULL) {
        ...
        uart->CTRL |= EF_UART_CTRL_REG_EN_MASK;   // set the enable bit to 1 at the specified offset
...
EF_DRIVER_STATUS EF_UART_enableRx(EF_UART_TYPE_PTR uart){
    ...
    if (uart == NULL) {
        ...
        uart->CTRL |= EF_UART_CTRL_REG_RXEN_MASK; // set the enable bit to 1 at the specified offset
...
EF_DRIVER_STATUS EF_UART_enableTx(EF_UART_TYPE_PTR uart){
    ...
    if (uart == NULL) {
        ...
        uart->CTRL |= EF_UART_CTRL_REG_TXEN_MASK; // set the enable bit to 1 at the specified offset
...
EF_DRIVER_STATUS EF_UART_enableLoopBack(EF_UART_TYPE_PTR uart){
    ...
    if (uart == NULL) {
        ...
        uart->CTRL |= EF_UART_CTRL_REG_LPEN_MASK; // set the enable bit to 1 at the specified offset
...
EF_DRIVER_STATUS EF_UART_enableGlitchFilter(EF_UART_TYPE_PTR uart){
    ...
    if (uart == NULL) {
        ...
        uart->CTRL |= EF_UART_CTRL_REG_GFEN_MASK; // set the enable bit to 1 at the specified offset
...
EF_DRIVER_STATUS EF_UART_setStopBits(EF_UART_TYPE_PTR uart, bool is_two_bits){
    ...
    if (uart == NULL) {
        ...
        if (is_two_bits){
            uart->CFG |= EF_UART_CFG_REG_STP2_MASK; // set the enable bit to 1 at the specified offset
        } else {
...
EF_DRIVER_STATUS UART_Init(EF_UART_TYPE_PTR uart, uint32_t baud_rate, uint32_t bus_clock, uint32_t data_bits, bool two_stop_bits, enum parity_type parity, uint32_t timeout, uint32_t rx_threshold, uint32_t tx_threshold) {
    ...
    if (status == EF_DRIVER_OK) {status = EF_UART_setStopBits(uart, two_stop_bits);} else {}
